...
class Transaksi extends MY_Controller {
    ...
    public function store()
    {
        ...
        $data_transaksi = [
            ...
            'transaksi_total' => $this->input->post('grand_total'),
            'transaksi_kembalian' => $this->input->post('kembalian'),
    ...
    //     }
    //     $data['transaksi'] = $transaksi;
    //     $this->load->view('transaksi/cetak_struc',$data);    
    // }

    public function print($id)
    {
        $data['profil'] = $this->Dashboard_model->get_profil();
        $data['trans'] = $this->Transaksi_model->trans($id);
        $data['detail'] = $this->Transaksi_model->detail($id);
        $data['all'] = $this->Transaksi_model->all2($id);
        $this->load->view('transaksi/cetak_struc',$data); 
    }
}
